...
\begin{figure}[H]
  \centering
  \includegraphics[width=100mm]{strategy}
...
The \textsc{Interpreter} pattern is used when a recurring problem within a platform arises. When this is the case, it might be fruitful to define a simple language to solve these types of problems \cite{gang_of_four}. Regarding AOMs, this pattern, in conjunction with the \textsc{Strategy} pattern described in \ref{sec:strategy_pattern}, is used to allow users to express complex restrictions on the values of properties present in the system. \textbf{ESCREVER AQUI QUALQUER COISA PARA INTRODUZIR A FIGURE} Fig.~\ref{fig:interpreter_pattern}

\begin{figure}[H]
  \centering
  %\includegraphics[width=100mm]{interpreter}
