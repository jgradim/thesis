...
Up until now, there has been very little work regarding adaptive systems on the web. There are a plethora of websites\cite{a,b,c,d,e} which offer varying degrees of customization, such as repositioning content on the webpage, or adding new content (usually small applications known as widgets), or changing the overall look of the application (more commonly known as \textit{skinning} or \textit{theming}).
...
As such these are not true \textit{adaptive systems} as per the definition gave by Joseph Yoder
